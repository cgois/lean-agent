...
4. **Overall theorem**
   - Lean theorem statement:
     ```lean
     -- paste the current target theorem statement from Problem.lean here
     ```
...
Keep this synchronized with `Problem.lean`.

```lean
-- paste the current target theorem statement exactly as it appears in Problem.lean